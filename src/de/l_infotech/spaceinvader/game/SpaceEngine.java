...
public class SpaceEngine extends Thread implements SensorEventListener,
	...
	public final String TAG_ENEMY = "Enemy";
	...
	private static final int STAGE_FAKTOR = 4;
	...
	public static final int GAME_INITIALISATION_WAIT_TIME = 1250; // lower ->
																	// faster
																	// start
	...
	public void run() {
		...
		while (isRunning) {
			if (!pause) {
				// clear the Field
				this.insertGamefield(StaticMatrix.CLEAR, 0, 0);
				...
				if (enemys.size() == 0) {
					...
					this.pause();
					score += (STAGE_VALUE * STAGE_FAKTOR);
					stage++;
					notifyScoreListener();
					notifyStageListener();
					initEnemy();
			...
			for (int z = 0; z < enemys.size(); z++) {
				...
				if (value.isAlive()) {
					...
					insertGamefield(value.getDestroyGraphics(),
							...
							value.getCoordinates().y0);
					enemys.remove(z);
					score += ((ENEMY_VALUE * stage) + (maxEnemys - enemys.size()));
					notifyScoreListener();
					sb.playSound(Soundboard.EXPLOSION);
				}
			}
